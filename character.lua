...
-- 各角色拥有技能 --
char_juese_jineng = {    -- 体力上限, 阵营, 是否为传统主公, 技能，是否加入将池
...
if enable_debug_wujiang then
	char_juese_jineng["孙笑川"] = {{4, 4}, "神", false, {"挑衅", "甘露", "红颜", "天香"}, "男", {"", "", "锁定", ""}, true}
end
...
-- 各角色武将牌 --
char_wujiang = {"刘备", "刘禅", "曹操", "曹丕", "孙权", "孙策", "张角", "袁绍", "董卓", "关羽", "张飞", "赵云", "马超", "诸葛亮", "黄月英", "黄忠", "魏延", "庞统", "卧龙诸葛", "孟获", "祝融", "姜维", "司马懿", "郭嘉", "张辽", "甄姬", "夏侯惇", "曹仁", "许褚", "夏侯渊", "荀彧", "典韦", "徐晃", "邓艾", "张郃", "张春华", "甘宁", "黄盖", "周瑜", "陆逊", "大乔", "吕蒙", "孙尚香", "周泰", "太史慈", "鲁肃", "孙坚", "张昭张紘", "凌统", "吴国太", "颜良文丑", "吕布", "貂蝉", "华佗", "庞德", "蔡文姬", "左慈", "公孙瓒", "神曹操", "神司马懿", "神吕蒙", "神周瑜", "曹彰", "廖化", "灵雎", "SP貂蝉", "SP姜维", "袁术", "贾诩", "于禁", "小乔"}
	...
	for i = 1, 5 do
	    if char_juese[i].shenfen == "主公" and i ~= char_current_i then
			...
			for j = 1,#char_juese_jineng[char_juese[i].name][4] do
				local skillname = char_juese_jineng[char_juese[i].name][4][j]
...
function char_calc_distance(_ID_s, _ID_d, horse_ignore, delta)
	...
	if char_juese[_ID_s].siwang == true or char_juese[_ID_d].siwang == true then
		return 10
	end
